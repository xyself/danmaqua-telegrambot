...
class DanmakuHandler extends BaseHandler {
    ...
    _on_notice_msg(client, message) {
        try {
            // 获取通知内容
            const noticeText = message.msg_common || message.msg_self || JSON.stringify(message);
            
            // 完全忽略带有特定礼物关键词的系统通知
            if ((noticeText.includes('<%') && noticeText.includes('%>送给') && noticeText.includes('点击前往')) || 
                (noticeText.includes('系统通知:') && noticeText.includes('送给') && noticeText.includes('个') && noticeText.includes('点击前往'))) {
                // 完全忽略这类礼物通知，不记录日志
                return;
            }
            
            // 记录其他系统通知但不转发
            this.source.logger.debug(`[${this.roomId}] 系统通知: ${noticeText}`);
        } catch (e) {
